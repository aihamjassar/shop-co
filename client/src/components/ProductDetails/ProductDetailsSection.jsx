...
import { addToCart, getCart } from "../../store/thunks/cartThunk";
...
export const ProductDetailsSection = ({ product }) => {
  ...
  const isAdding = status.add === "loading";

  const handleAddToCart = async () => {
    if (!isAuthenticate) {
      uiDispatch({ type: "OPEN_SIGNIN_MODAL" });
      return;
    }
    ...
    try {
      ...
      await reduxDispatch(getCart()).unwrap();
  ...
  return (
    <div className="px-5 md:px-8 py-5 mb-16">
      <div className="text-black/60">
        ...
        &gt;{" "}
        <Link to="/shop" className="transition-colors duration-300 hover:text-blue-700">
          Shop
        </Link>{" "}
        &gt; {name}
      </div>
      <div className="flex flex-col md:flex-row gap-5 mt-5">
        ...
        <div className="w-full space-y-4">
          ...
          <div className="flex items-center gap-0.5">
            ...
            <span className="text-sm text-black/60">({product.ratingsQuantity || 0} reviews)</span>
          </div>
          <div className="flex items-center">
            <span className="text-2xl font-bold">${salePrice.toFixed(2)}</span>
            {discount > 0 && (
              <>
                <span className="text-2xl font-bold text-black/30 line-through mx-2.5">${price.toFixed(2)}</span>
                <span className="w-12 text-[#FF3333] text-[14px] bg-red-100 p-1 px-2 rounded-2xl">-{discount}%</span>
              </>
            )}
          </div>
          ...
          <div className="flex gap-3">
            {colors.map((color) => {
              ...
              return (
                ...
                >
                  {selectedColor === color && <Check className={isLightColor ? "text-black" : "text-white"} />}
                </button>
              );
            })}
          </div>
          ...
          <div className="flex gap-2.5">
            <div className="flex">
              ...
              <span className="block size-8 leading-7 text-center bg-gray-200">{quantity}</span>
              <button
                type="button"
            ...
            <button
              ...
              className="flex-auto max-w-96 h-8 rounded-2xl bg-black text-white cursor-pointer hover:bg-black/90 transition-colors disabled:opacity-50"
            >
